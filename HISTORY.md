# Historical README through 0.18.0

These dated notes are not the current capability contract. See README.md and WORDS.md.

# Forth 栈式解释器

可嵌入、具执行预算的 Forth 子集。本地开发版 0.18.0，供比较和代码审查；尚未作为完整竞赛作品提交。
...
新增 ABS、MIN、MAX、/MOD、*/、*/MOD、2*、2/、INVERT、XOR、LSHIFT、RSHIFT、U<、U>、WITHIN、<>、0<>。*/ 和 */MOD 使用 64 位中间乘积，避免先做 32 位乘法产生截断；例如 `100000 100000 100000 */` 得到 100000。

/MOD 和 */MOD 先压余数，再压商；有符号除法向零截断，余数与被除数同号。2/ 是算术右移（负奇数向下取整），RSHIFT 是逻辑右移；位移量必须在 0–31，超出报错。商超出有符号 cell 范围或除零时报错。ABS 的最小负 cell 按 32 位回绕保持 -2147483648。WITHIN 使用无符号差值，支持跨越 cell 边界的半开区间，同上下界为空区间。

本轮仅运行 `integer extension*` 的 5 组新增 JS 测试，覆盖宽中间值、正负商余数、位操作、回绕区间、编译调用及错误范围。未重复全套或打包；之前 26 个 Gforth 对照是对应报告中引擎指纹的快照，不覆盖本轮新增运算。不同 Forth 可选择不同除法舍入方式，不能将这一选择视为所有上游均一致。
